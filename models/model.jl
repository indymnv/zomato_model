...
begin
	...
	max_depth_range = range( dt,  :max_depth, lower = 1, upper = 30)
	
	lmTuneModel = TunedModel(model=dt,
...
begin
	...
	#max_depth_range = range(rf,  :max_depth, lower = 1, upper = 10)
	n_trees_range = range(rf, :n_trees, lower=9, upper=15)
	
	rfTuneModel = TunedModel(model=rf,
                          resampling = CV(nfolds=6, shuffle=true),
						  tuning = Grid(resolution=25),
                          range = [max_depth_range, n_trees_range],
                          measures=[mae, rms, rsquared]);
	
	rfMachine = machine(rfTuneModel, X_encoded, y)
	fit!(rfMachine, rows=train, verbosity=0)
	evaluate!(rfMachine, rows = train,
		  verbosity=0)
...
DataFrames = "~1.4.1"
MLJ = "~0.18.5"
MLJDecisionTreeInterface = "~0.2.5"
Plots = "~1.35.3"
"""
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "85025a4c9822d6529c61a7d68b9d0578893ee1e1"
